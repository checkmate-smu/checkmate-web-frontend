...
| **apiFetch** | `@/07-shared/api` | Spring Boot API 호출 |
...
// GET
const article = await apiFetch<Article>('/articles/123');
...
// apiFetch는 자동으로 응답 에러를 AppError로 변환
try {
  const data = await apiFetch('/analysis/123');
} catch (err) {
